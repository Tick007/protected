...
class ConstructcatalogController extends Controller
{
	...
	public function filters()
	{
		return array(
			...
			'CheckBrouser +tools, group',
			'SetTheme +tools, group',
		);
	}
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'list' and 'show' actions
				'actions'=>(isset(Yii::app()->params['authenticated_catalog'])==true)? array('notexist') :array('group', 'index', 'getgroups', 'getyears', 'info',  'getgroups2', 'getyears', 'getkpptypes', 'search', 'error', 'zamki', 'tools', 'adv', 'gettext', 'get_typs_kpp', 'searcheasylift'),
			),
	...
	public function filterCheckKppKrepl($filterChain){
	    ...
	    if(isset(Yii::app()->params['filters']) && isset(Yii::app()->params['typs_krepleniya'])){
	...
	*/
	...
	public function actionGetyears(){////////Аджакс функция для выборке детей
			...
			if (isset($model)) {
					...
					////////////Это классика
					//$models_list=CHtml::tag('option', array('value'=>0), iconv( "UTF-8", "CP1251", "Модель"));
					$models_list=CHtml::tag('option', array('value'=>'0'), "выбор года");
					for($k=0; $k<count($models); $k++)
							{
	...
	///////////Метод для сайта easylift. Вытаскивет товары по категории и году (определенное значение в characteristics_values)
	/**
	 1. ПОлучаем ИД товара  используя значение группы и года (characteristic_value)
	 2. Выбираем фотки и инструкции (иструкции в основном запросе)
	 3. Рендерим вьюху и отдаем по ajaх
	 */
	public function actionSearchEasylift(){

	    //print_r($_POST);
	    $model = Yii::app()->getRequest()->getParam('model', null);
	    $year = Yii::app()->getRequest()->getParam('year', null);
	    if($model!=null && trim($model!='') && is_numeric($model)==true && $year!=null && trim($year!='')){
	        $criteria=new CDbCriteria;
	        ...
	        $criteria->select=array( 't.id', 't.product_name', 't.product_article');
	        $criteria->condition="t.category_belong = :cat ";
	        $criteria->order=" t.sort, t.product_name ";
	        //$criteria->join =" LEFT JOIN ( SELECT picture_product.id, product, picture, ext, comments FROM picture_product JOIN pictures ON pictures.id=picture_product.picture WHERE is_main=1) picture_product ON picture_product.product = t.id  ";
	        $criteria->join =" LEFT JOIN characteristics_values cv ON (cv.id_product = t.id) ";
	        $criteria->addCondition("t.product_visible = 1");
	        $criteria->addCondition("cv.value = :year");
	        $criteria->params =  array(':cat'=>$model, ':year'=>$year);
	        $models = Products::model()->with('files', 'char_val', 'belong_category')->findAll($criteria);
	        
	        if(is_null($models)==false){
	            foreach ($models as $model){
	                $products_ids[]=$model->id;
	                $products_list[$model->id]= array(
	                        'id'=>$model->id,
	                        'name'=>$model->product_name, 
	                        'article'=>$model->product_article,
	                        'year'=>$year, 
	                        'model'=>$model->belong_category->category_name,
	                        'brand'=>$model->belong_category->parent_categories->category_name,
	                        'files'=>$model->files,
	                        //'images'=>$model->
	
	                    );
	                //$products_list[]=$model->id;
	            }
	            if(is_array($products_ids) && is_null($products_ids)==false){
	                 ///////////Выбираем фотки товара
	                $connection = Yii::app()->db;
	                $query = "SELECT picture_product.id, pictures.id, pictures.ext, pictures.type, picture_product.product FROM
  pictures JOIN picture_product ON picture_product.picture = pictures.id WHERE picture_product.product IN (".implode($products_ids).")
 ORDER BY picture_product.is_main, picture_product.id DESC";
	                $result = $connection->createCommand($query);
	                $additional_pictures = $result->query()->readAll();
	                
	                if($additional_pictures!=null){
	                    foreach ($additional_pictures as $value) {
	                        //print_r($value);
	                        if(isset($products_list[$value['product']])) $products_list[$value['product']]['images'][]=$value;
	                    }
	                }
	            }
	            
	            $ajax_content = $this->renderPartial('easylift', array('models'=>$products_list), false);
	            
	            //print_r($ajax_content);
	            echo $ajax_content;
	        }
