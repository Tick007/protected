...
class PageController extends Controller
{
	...
	public function filters()
	{
		...
		//	'loadPageNameAllias  + show',
			...
			'CheckBrouser +byalias, city, listinrubric',
			'SetTheme +byalias, city, listinrubric',
		);
	}
	...
	public function accessRules()
	{
		...
		return array(
			array('allow',  // allow all users to perform 'list' and 'show' actions
				'actions'=>array('list','show', 'faq', 'sapearticle'),
				'users'=>array('*'),
			),
		);
	}
	...
	public function actionSapearticle(){
		$this->layout="sape";
		$this->render('sape');
	}
	...
	public function actionList()
	{
		...
		//else $section_id = 0;
		$rubric_id =  Yii::app()->getRequest()->getParam('rubric_id', NULL);
		...
		else $criteria->condition=' t.active=1 AND t.section NOT IN (2,3) ';
		...
		if(isset($rubric_id)){
			$criteria->addCondition("t.rubric = :rubric_id");
	...
	public function actionCity(){///////////////////Вытаскиваем дилеров по городу
		...
		////////////Костыль для мобильной версии по письму от Романа
		if(isset(Yii::app()->params['mobile_theme']) && Yii::app()->params['mobile_theme']== Yii::app()->theme->name){
			if($regid==560) $regid=array(559, 560);
		}
		...
		if (isset($models) AND empty($models)==false) {
			if(Yii::app()->request->isAjaxRequest)	{
				$this->renderPartial('map/dealers', array('models'=>$models));
			}
			else{
				$this->render('map/dealers', array('models'=>$models));
			}
		}
	...
	private function get_city_models($regid){
		...
		elseif(is_array($regid) && empty($regid)==false) {
			$criteria->condition="t.category_belong IN (".implode(',', $regid).")" ;
		}
		 $criteria->join ="
LEFT JOIN ( SELECT id, product, picture FROM picture_product WHERE is_main=1) picture_product ON picture_product.product = t.id  ";
	...
	/*
	////////////////////////////////////////Вытаскиваем дилера, id - идентификатор товара (таблица products)
	...
	*/
	...
	public function actionListinrubric($rubrika){
	    
	    //print_r($_GET);
	    
	    $section=Yii::app()->getRequest()->getParam('section');
	    
	    $page_rubrics = Page_rubrics::model()->findByAttributes(array('alias'=>$rubrika));
	    if($page_rubrics!=NULL){
	        
	        if(isset($section)) {
	            $sect = Page_sections::model()->findByAttributes(array('alias'=>$section));
	        }
	        
	        
	        $criteria=new CDbCriteria;
	        $criteria->condition="t.rubric = :rubric AND t.active = 1" ;
	        
	        $criteria->order="t.creation_date DESC" ;
	        
	        if(isset($sect) && $sect!=NULL) $criteria->condition.=" AND t.section = ".$sect->id;
	        
	        $criteria->params=array(':rubric'=>$page_rubrics->id);
	        ...
	        $this->render('rubric', array('page_rubrics'=>$page_rubrics, 'models'=>@$models, 'pages'=>$pages));
	    }
	    else{
