	...
	public $myselect_data;
		...
		public function rules(){
			return array(
				...
				array('first_name, client_email, client_email_copy, client_tels, delivery_method, payment_method', 'required','on'=>'nomail'),
				array('first_name, second_name, client_email, client_email_copy, client_tels, delivery_method, payment_method, last_name,client_post_index', 'required', 'on'=>'mail'), 
	...
	function CheckEnter(){
			...
			*/
			...
			private function send_mail($order_id){///////////////Отправка сообщений
						...
						$msg_body = "Заказ №".$order_id." от ".$this->first_name.' '.$this->second_name.' '.$this->last_name."(".$this->client_email.")<br>\r\n";
					  ...
					  $msg_body.="<table  border=\"0\" cellspacing=\"1\" cellpadding=\"1\" bgcolor=\"#ffffff\">";
						...
						for ($i=0; $i<count($models); $i++) {/////////Перебор содержимого заказ
							if (trim($models[$i]->field_id)) {
								$msg_body.="<tr bgcolor=\"#FFFFFF\"><td>&nbsp;".CHtml::link($models[$i]->contents_name, array('product/details', 'pd'=>$models[$i]->contents_product)).",&nbsp;&nbsp; http://".$_SERVER['HTTP_HOST'].Yii::app()->createUrl('product/details', array( 'pd'=>$models[$i]->contents_product))."</td>";
