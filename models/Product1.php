...
if ($sgp==NULL AND trim($alias)) {
	...
	$this->query.=" categories.category_name  AS gr, price_list.price_with_nds, round(products.product_price,2) AS price_card, 	  price_list2.price_with_nds AS price_with_nds2, products.category_belong, products.product_html_description";
		...
		$this->query.=", store".$kk.".quantity AS prihod_store".$kk.",  0 AS rashod_store".$kk."  ";
		 ...
		 $this->query.="	, picture_product.picture AS icon , categories.alias AS catalias ";
		  ...
		  $this->query.="FROM  (SELECT products.id, products.product_article,  products.category_belong, products.product_price, products.nds_out, products.product_visible, products.product_name , products.product_parent_id, products.product_vitrina, products.product_html_description, products.product_sellout, products.product_new, number_in_store FROM products 
			 UNION
		...
		//	 LEFT JOIN  ";
		  for($k=0;$k<count($this->stores_id);$k++) {
		$kk=$k+1;
					   $this->query.=" LEFT JOIN (SELECT parent_categories.category_name AS sgr, products.product_name, ostatki.quantity, categories.category_name AS gr, products.id
			FROM products
			LEFT JOIN (
		...
		public $char_values_list;
						*/
						...
						//print_r($list_caracteristic_values);
						 ...
						 $this->query.= " (SELECT products.id, products.product_article, products.category_belong, products.product_price, products.nds_out, products.product_visible, products.product_name , products.product_parent_id, products.product_vitrina, products.product_html_description FROM products UNION SELECT products.id, products.product_article, categories_products.group AS category_belong, products.product_price, products.nds_out, products.product_visible, products.product_name , products.product_parent_id, products.product_vitrina, products.product_html_description FROM products JOIN categories_products ON categories_products.product = products.id) ";
		...
		if(isset(Yii::app()->params['dont_show_ostatki_null']) AND Yii::app()->params['dont_show_ostatki_null']==true) $this->query.= " AND products.number_in_store>0 ";
			...
			//echo $this->query;
			if(isset($this->sort_order)) $this->query.=" ORDER BY ". $this->sort_order;
	...
	public function char_list() {/////////////////////Список характеристик группы для фильтров
	 ...
	 private function list_of_distinct_values($caract_id) {
			...
			WHERE characteristics_values.value <>  '' AND products.category_belong = ".$this->show_group." AND characteristics_values.id_caract =$caract_id
			UNION 
			
			SELECT DISTINCT characteristics_values.value, characteristics_values.id_caract
			FROM characteristics_values
			JOIN categories_products ON categories_products.product = characteristics_values.id_product
			WHERE characteristics_values.value <>  '' AND categories_products.group = ".$this->show_group." AND characteristics_values.id_caract =$caract_id
			
			
			";
			
			}
	 		else $query = "SELECT DISTINCT characteristics_values.value, characteristics_values.id_caract
			FROM characteristics_values
			JOIN products ON products.id = characteristics_values.id_product
