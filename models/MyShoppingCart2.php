<?php
//////////////Работает только с кукисами. По сути наверное от MyShoppingCart
class MyShoppingCart2 {
...
private $history;//////История просмотров
			...
			function __construct($pricevar_id=null, $num_to_basket=1){
				
				///////////////////////////Инициализировали из куки
				$this->initializeCookie(); 
				if($pricevar_id!=null) $this->addToOrder($pricevar_id, $num_to_basket);
			 ...
			 * @param int $tovar_id - уже на самом деле pricevar_id
			 ...
			 * @param int $price_var id в таблице pricevariations
			 * @param int $kol количество
			 */
			public function changeOrder($price_var, $kol){
				...
				*/
				if($kol==0 && isset($this->order[$price_var])) unset ($this->order[$price_var]);
				else {
				    $this->order[$price_var] = $kol;
				}
