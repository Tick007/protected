...
class Product extends CFormModel {
	...
	function __construct($sgp=NULL){ /////////////добавил специально чтобы вытаскивать товары задавая группу вручную (ольгин сайт)
		$time1 = microtime(true);
			...
			$debug = Yii::app()->getRequest()->getParam('debug', NULL);
			...
			/////////////////////////////Инициализируем склады
			 $this->query1= "SELECT id, name FROM stores WHERE kontragent_id = ".Yii::app()->GP->GP_self_contragent." AND show_in_html=1   AND id <> ".Yii::app()->params['reserv_sklad']." ORDER BY is_main DESC";
			...
			*/
			...
			//echo '1';
		/*	
				echo 'До проверки на режим таблиц: ';
		$tme2 = microtime(true);	
	echo $tme2 - $time1;
	echo '<br>';
	echo $pd;
	*/
			...
			if(isset(Yii::app()->params['use_temp_tables']) AND Yii::app()->params['use_temp_tables']==true AND $pd==NULL) {
			//echo '2';
			...
			//echo microtime(true)." ";
			//echo strtotime($extra_res[0]['Create_time'])." ";
			//echo $created_ago;
		...
		if($this->show_group>0)  $query.=" WHERE categories_products.group = ".$this->show_group; 
			...
			DELETE FROM  ".$this->char_val_temp_table_name." ;
			 INSERT INTO  ".$this->char_val_temp_table_name." (value_id,	id_caract,	id_product,	value ) SELECT value_id,	id_caract,	id_product,	value FROM characteristics_values WHERE id_product IN(SELECT id FROM ".$this->temp_table_name.") ";
				...
				//echo 'werewrewr<br>';
				if(is_numeric($pd)==true) {
					
					$this->temp_table_name = " (SELECT * FROM products WHERE products.id= ".$pd."     ) ";
					$this->char_val_temp_table_name = " (SELECT * FROM characteristics_values WHERE  characteristics_values.id_product = ".$pd."  )  characteristics_values";
					$this->ostatki_temp_table_name = " (SELECT * FROM  ostatki_trigers WHERE ostatki_trigers.tovar = ".$pd."  )  ostatki_trigers ";
				}//////if(is_numeric($pd)==true) {
				else {
			...
			///////////////////Красивые филтры пришедшие в строке
			$filtr_id =  Yii::app()->getRequest()->getParam('filtr_id');
			$filtr_value =  Yii::app()->getRequest()->getParam('filtr_value');
			if(isset($filtr_id) AND isset($filtr_value) AND isset($this->char_values_list[$filtr_id])) {
				 //$this->char_list();
				//print_r($this->char_values_list);
				$values = $this->char_values_list[$filtr_id];
				$values_keys = array_flip($values);
				foreach($values_keys as $key=>$value){
				$values_keys2[mb_strtolower($key, 'utf-8')]=$value;
				}
					if(isset($debug) ) print_r($values_keys2);
				$value =  mb_strtolower($filtr_value, 'utf-8');
					if(isset($debug) ) echo $value;
				if(isset($values_keys2[$value])) {
						if(isset($debug) ) echo '<br>v = '.$values_keys2[$value];
					$this->cfid_arr[$filtr_id]=array($values_keys2[$value]=>1);
				}
				
			}///////////if(isset($filtr_id) AND isset($filtr_value)) {
	...
	//	echo '<br>'		;
		...
		$this->query="SELECT DISTINCT products.id, products.product_article,  parent_categories.category_name  AS sgr, 
				products.product_html_description AS product_html_description ,  products.product_sellout, products.sellout_price, 
				products.product_new,  number_in_store,   ";
	...
	if(isset(Yii::app()->params['group_stores']) AND Yii::app()->params['group_stores']==true) {
		$this->query.=" categories.alias, categories.path, categories.category_name  AS gr, round(products.product_price,2) AS price_card, store1.store_price AS price_with_nds,  products.category_belong, products.product_html_description, products.product_price_old ";
		...
		$this->query.=" categories.alias, categories.path, categories.category_name  AS gr, products.product_price AS price_with_nds, round(products.product_price,2) AS price_card, products.category_belong, products.product_html_description, products.product_price_old ";///////////////////Жутко тормозит
		for($k=0;$k<count($this->stores_id);$k++) {
			$kk=$k+1;
			$this->query.=", store".$kk.".quantity AS prihod_store".$kk.",  0 AS rashod_store".$kk."  ";
			$ostatki_parts[] =  " IFNULL(store".$kk.".quantity,0) ";
		...
		//	 LEFT JOIN  ";
			if(isset(Yii::app()->params['group_stores']) AND Yii::app()->params['group_stores']==true) {
				//  for($k=0;$k<count($this->stores_id);$k++) {
					$k=0;
					$kk=$k+1;
					$this->query.=" LEFT JOIN ( SELECT 	tovar,  SUM(IFNULL(quantity, 0)) as  quantity,  MAX(IFNULL(store_price,0)) as store_price FROM ";
					$this->query.= "  ".$this->ostatki_temp_table_name ;
					if(isset(Yii::app()->params['reserv_sklad'])) $this->query.= " WHERE store <> ".Yii::app()->params['reserv_sklad'] ;
					$this->query.= "  GROUP BY tovar ) ";
					$this->query.= "   store".$kk;
					//if ($k>0) $this->query.= " ON  (products.id = store$kk.tovar  )"; 
					if ($k==0)$this->query.= "  ON ( products.id = store1.tovar  )";
		...
		////////////////Костылек для того что бы была фильтрация по строке ?vendor=name
		///нужно бы еще перенести тоже самое в countingquery
		$vendor=Yii::app()->getRequest()->getParam('vendor') ;
		if(isset($vendor)){
			//echo $vendor.'<br>';
			$list_caracteristic_values = $this->char_values_list[Yii::app()->params['vendor_char_id']];
			//print_r($list_caracteristic_values);
			if(isset($list_caracteristic_values)){
				$vendor_num = array_search(strtolower($vendor), array_map('strtolower', $list_caracteristic_values) );
				//echo '$vendor_num = '.$vendor_num.'<br>';
				if(is_numeric($vendor_num)){
					$this->cfid_arr[Yii::app()->params['vendor_char_id']]=array($vendor_num=>$vendor_num);
				}
			}
		}
		...
		*/
		...
		//echo '<br>'			;
			...
			//echo '<br>'.$this->char_val_temp_table_name.'<br>';
			...
			FROM ".$this->char_val_temp_table_name."  JOIN (SELECT * FROM products WHERE product_parent_id >0) child_products ON child_products.id = characteristics_values.id_product GROUP BY id_product ORDER BY characteristics_values.value_id) product_attribute ON product_attribute.id_product = products.id ";
		...
		if (isset($rows) AND $search!=NULL AND trim($search)!='' AND trim($search)!='' AND empty($rows)==false) $this->query.= "  AND products.id IN (".implode(',', $rows).") ";
		/*
		print_r($search);
		var_dump($search);
		echo 'werwerwer<br>';
		var_dump($search!=NULL AND trim($search)!='' AND htmlspecialchars(strip_tags($search))!='' AND  ( isset($rows) AND empty($rows)==true) OR isset($rows)==false);
		
		echo '<br>$search!=NULL';
		var_dump($search!=NULL);
		echo '<br>trim($search)!=""';
		var_dump(trim($search)!='');
		echo '<br>htmlspecialchars(strip_tags($search))!=""';
		var_dump(htmlspecialchars(strip_tags($search))!='');
		echo '<br> isset($rows) AND empty($rows)==true) ';
		var_dump(isset($rows) AND empty($rows)==true);
		
		echo '<br>isset($rows)==false';
		var_dump(isset($rows)==false);
		
		echo 'werwerwer<br>';
		*/
		if($search!=NULL AND trim($search)!='' AND htmlspecialchars(strip_tags($search))!='' AND  ( isset($rows) AND empty($rows)==true OR isset($rows)==false)) $this->query.= "  AND  (products.product_article LIKE '%".htmlspecialchars(strip_tags($search))."%' OR products.product_name LIKE '%".htmlspecialchars(strip_tags($search))."%' ) ";
		...
		//echo '<br>'			;
			...
			if(isset($this->product_sellout) ) $this->query.= " AND products.product_sellout = 1 ";
		...
		//	if (!isset($this->pd) OR is_numeric(trim(htmlspecialchars($this->pd)))==false ) 
		//	{

				
				 if($this->offset>=0 AND $this->limit>0 )  $this->query.=" LIMIT :offset,:limit";
			//}
	...
	public function run_query() {
			...
			if($this->offset>=0 AND $this->limit>0 )
			{
	...
	//echo $query;
	 ...
	 if (count($rows) > 0) return 1;
		...
		if (isset($rows) AND isset($search) AND trim($search)!='' AND empty($rows)==false) $query.= "  AND products.id IN (".implode(',', $rows).") ";
		//if(isset($search) AND  ( isset($rows) AND empty($rows)==true) OR isset($rows)==false) $query.= "  AND  (products.product_article LIKE '%".htmlspecialchars(strip_tags($search))."%' OR products.product_name LIKE '%".htmlspecialchars(strip_tags($search))."%' ) ";
		if($search!=NULL AND trim($search)!='' AND htmlspecialchars(strip_tags($search))!='' AND  ( isset($rows) AND empty($rows)==true OR isset($rows)==false)) $query.= "  AND  (products.product_article LIKE '%".htmlspecialchars(strip_tags($search))."%' OR products.product_name LIKE '%".htmlspecialchars(strip_tags($search))."%' ) ";
	...
	public function char_list() {/////////////////////Список характеристик группы для фильтров
			...
			if(isset(Yii::app()->params['group_characteristics_mode']) AND Yii::app()->params['group_characteristics_mode']=='multi'){
				$query = "SELECT caract_id, caract_name FROM characteristics JOIN  characteristics_categories ON 
						characteristics_categories.characteristics_id  = characteristics.caract_id ";
	 ...
	 private function list_of_distinct_values($caract_id) {
			...
			WHERE characteristics_values.value <>  '' 	AND products.product_visible = 1 AND characteristics_values.id_caract =$caract_id ORDER BY characteristics_values.value 
			";
			//echo $query.'<br>';
			$connection = Yii::app()->db;
			$command=$connection->createCommand($query);
