...
class CartNew extends CFormModel {
	...
	public function rules() {
		return array (
				...
				isset ( Yii::app ()->params ['cart'] ['rules'] ['nomail'] ) ? Yii::app ()->params ['cart'] ['rules'] ['mail'] : array (
				...
				array (
						'client_email',
						'email',
						'message' => "Ввидете правильный email" 
				),
				array (
				...
				array (
						...
						'message' => 'Не совпадают адреса электронной почты' 
				),
				array (
		...
		if (is_array ( $this->products ) == true and empty ( $this->products ) == false) {
			...
			if (isset ( $models )) {
				...
				for($i = 0, $c = count ( $models ); $i < $c; $i ++) {
					...
					$actual_price = FHtml::cardDiscountPrice($discount, FHtml::getProductPrice ( $models [$i],  $models [$i]->id) );
...
;
				...
				if (isset ( $productslist ) and is_array ( $productslist ) == true and empty ( $productslist ) == false) {
					...
					if (isset ( $records )) {
						for($i = 0; $i < count ( $records ); $i ++)
							$this->products_attributes [$records [$i] ['id_product']] [$records [$i] ['id_caract']] = $records [$i] ['value'];
					}
				...
				for($i = 0; $i < count ( $characteristics1 ); $i ++) {
					$this->characteristics_array [$characteristics1 [$i]->caract_id] = array (
							'char_type' => 4,
							'caract_name' => $characteristics1 [$i]->caract_name,
							'is_main' => $characteristics1 [$i]->is_main 
					);
	 ...
	 */
	public function saveDBOneClickOrder(Products $product, OneClickOrderForm $cart) {
		$new_order_num = null;
		$discount = FHtml::getClientDiscount ();
		$this->client_email = $cart->email;
		$this->first_name = $cart->name;
		$this->client_city = $cart->city;
		$this->client_tels = $cart->tel;
		$this->primechanie = 'Заказ в 1 клик';
		try {
			$this->makeorderSimple ( false );
			if ($this->created_order != null) {
				$new_order_num = $this->created_order;
				
				// ///////////тут нужно запихать в заказ товары
				$rec = new OrderContent ();
				$rec->id_order = $new_order_num;
				$rec->quantity = 1;
				// $rec->contents_price = FHtml::getProductPrice($product);
				$rec->contents_price = FHtml::cardDiscountPrice ( $discount, FHtml::getProductPrice ( $product ), $product->id );
				$rec->contents_product = $product->id;
				$rec->contents_name = $product->product_name;
				$rec->contents_article = $product->product_article;
				try {
					$rec->save ();
				} catch ( Exception $e ) {
	...
	public function makeorderSimple($send_mail = true) { // //////////создание заказа
		...
		$AR_Order->id_client = FHtml::checkClientByemail ( trim ( $this->client_email ), true, array (
		...
		/*
		 * echo '$this->order_adress1;'.$this->order_adress1;
		 * echo '<pre>';
	...
	private function send_mail($order_id, $client_id) { // /////////////Отправка сообщений
		...
		$msg_body .= "Клиент: " . $this->second_name . ' ' . $this->first_name . ' ' . $this->last_name . "<br>\r\n";
		$msg_body .= "Телефон: " . $this->client_tels . "<br>\r\n";
		$msg_body .= "Индекс: " . $this->client_post_index . "<br>\r\n";
		
		$kladr = World_adres_cities::model ()->with ( 'region' )->findByPk ( $this->client_city );
		if ($kladr != NULL) {
			if ($kladr->region != null)
				$msg_body .= "Область (республика, край): " . $kladr->region->name . "<br>\r\n";
			$msg_body .= "Город: " . $kladr->name . "<br>\r\n";
		}
		$msg_body .= "Доп город: " . $this->order_adress2 . "<br>\r\n";
		// $msg_body.="Адрес: ".$this->client_street.', д.'.$this->client_house.', кв.'.$this->client_apart."<br>\r\n";
		$msg_body .= "Адрес: " . $this->order_adress1 . "<br>\r\n";
		...
		if (isset ( Yii::app ()->params ['cart'] ['product_yii_path'] ))
			$prod_path = Yii::app ()->params ['cart'] ['product_yii_path'];
		else
			$prod_path = 'catalog/info';
		if (isset ( Yii::app ()->params ['cart'] ['product_id_label'] ))
			$prod_id_label = Yii::app ()->params ['cart'] ['product_id_label'];
		else
			$prod_id_label = 'id';
		...
		foreach ( $this->products as $prod_id => $prod ) { // ///////Перебор содержимого заказ
			...
			$url = Yii::app ()->createAbsoluteUrl ( $prod_path, array (
					$prod_id_label => $prod_id,
					'alias' => $prod ['product'] ['category_alias'] 
			) );
			
			$price = FHtml::cardDiscountPrice($discount, $prod['product']['product_price'],$prod_id);
			
			$sum += round ( $prod ['num'] * $price, 2 );
			$amount += $prod ['num'];
			if (isset ( $prod ['product'] ['category_alias'] ))
				$msg_body .= "<tr bgcolor=\"#FFFFFF\"><td>" . CHtml::link ( $prod ['product'] ['product_name'], $url ) . "</td>";
			else
				$msg_body .= "<tr bgcolor=\"#FFFFFF\"><td>" . $prod ['product'] ['product_name'] . "</td>";
			$msg_body .= "<td><nobr>&nbsp;" . $prod ['num'] . " шт.</nobr></td>";
			$msg_body .= "<td><nobr>&nbsp;" . $price . " руб.</nobr></td>";
			$msg_body .= "</tr>";
		} // //////////for ($i=0; $i<count($order); $i++) {
		$msg_body .= "<tr bgcolor=\"#FFFFFF\">";
		$msg_body .= "<td>Итого</td>";
		$msg_body .= "<td>$amount</td>";
		$msg_body .= "<td>$sum руб.</td>";
		$msg_body .= "</tr>";
	...
	private function deliveryDetails() { // /////////////////достаем по коду типа 5:p детали доставки оплаты
		...
		if ($temp [0] == 'default') { // ////////ЕМС
			...
			$products_region = Products_regions::model ()->with ( 'productmodel' )->findByPk ( $temp [0] );
	...
	private function saveProducts($order_id, $delivery_prod_id = null) { // ////////////////Сохранение товаров в заказ
		if ($delivery_prod_id != null) { // /////Добавляем доставку
			...
			if ($deliv != null)
				$this->products [$delivery_prod_id] = array (
						'num' => 1,
		...
		if (empty ( $this->products ) == false)
			$discount = FHtml::getClientDiscount();
			foreach ( $this->products as $pid => $product_array ) {
				$rec = new OrderContent ();
				$rec->id_order = $order_id;
				$rec->quantity = $product_array ['num'];
				//$rec->contents_price = $product_array ['product'] ['product_price'];
				/////////Запись в БД при заказе
				$rec->contents_price = FHtml::cardDiscountPrice($discount, $product_array['product']['product_price'], $product_array['product']['id']);
