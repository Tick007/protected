...
class Product extends CFormModel {
	...
	function __construct($sgp=NULL){ /////////////добавил специально чтобы вытаскивать товары задавая группу вручную (ольгин сайт)
		$time1 = microtime(true);
			...
			/////////////////////////////Инициализируем склады
			 $this->query1= "SELECT id, name FROM stores WHERE kontragent_id = ".Yii::app()->params['self_contragent']." AND show_in_html=1   AND id <> ".Yii::app()->params['reserv_sklad']." ORDER BY is_main DESC"; 
	...
	*/
	...
	//	echo '<br>'		;
		...
		$this->query="SELECT DISTINCT products.id, products.product_article,  parent_categories.category_name  AS sgr, products.product_html_description AS product_html_description ,  products.product_sellout, products.sellout_price, products.product_new,  number_in_store, ";
	...
	if(isset(Yii::app()->params['group_stores']) AND Yii::app()->params['group_stores']==true) {
		...
		public $char_values_list;
						*/
						...
						//echo '<br>';
						if($this->sub_cat_ids!=NULL) $num = array_search($name, $this->sub_cat_ids); //////////порядковый номер в массиве id характеристик
						//echo '$num = '.$num.'<br>'; 
		...
		if (isset($rows) AND $search!=NULL AND trim($search)!='' AND trim($search)!='' AND empty($rows)==false) $this->query.= "  AND products.id IN (".str_replace(',,', ',',implode(',', $rows)).") ";
		/*
		print_r($search);
		var_dump($search);
		echo 'werwerwer<br>';
		...
		*/
		...
		//	{
				 ...
				 else if($this->offset==0 AND $this->limit>0 )  $this->query.=" LIMIT :limit";
			//}
	...
	public function run_query() {
			...
			if($this->limit>0 )
			{
				if($this->offset>=0)$result->bindValue(':offset', $this->offset);
	...
	//echo $query;
	 ...
	 if (count($rows) > 0) return 1;
		...
		if (isset($rows) AND isset($search) AND trim($search)!='' AND empty($rows)==false) $query.= "  AND products.id IN (".str_replace(',,', ',',implode(',', $rows)).") ";
	...
	public function CheckView(){ ////////////Проверяем режим вывода товаров
			    	 ...
			    	 $session->open();
					  ...
					  else $this->out_mode = Yii::app()->params['list_view'];
					  $session->close();
