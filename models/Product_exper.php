...
if ($sgp==NULL AND trim($alias)) {
	...
	$this->query.=" categories.category_name  AS gr, price_list.price_with_nds, round(products.product_price,2) AS price_card, 	  price_list2.price_with_nds AS price_with_nds2, products.category_belong, products.product_html_description";
		...
		$ostatki_parts[] =  " IFNULL(store".$kk.".quantity,0) ";
		 ...
		 $this->query.="	, picture_product.picture AS icon , categories.alias AS catalias ";
		
		  $this->query.="FROM  (SELECT products.id, products.product_article,  products.category_belong, products.product_price, products.nds_out, products.product_visible, products.product_name , products.product_parent_id, products.product_vitrina, products.product_html_description, products.product_sellout, products.product_new, number_in_store FROM products_temp  WHERE products.category_belong = ".$this->show_group."
			 UNION
			 SELECT products.id, products.product_article,  categories_products.group AS category_belong, products.product_price, products.nds_out, products.product_visible, products.product_name , products.product_parent_id, products.product_vitrina, products.product_html_description, products.product_sellout, products.product_new, number_in_store FROM products_temp  JOIN categories_products ON categories_products.product = products.id WHERE categories_products.group = ".$this->show_group."
			 ) products ";
		  
		  
		  $this->query.=" LEFT JOIN ( SELECT id, product, picture FROM picture_product WHERE is_main=1) picture_product ON picture_product.product = products.id  ";
		...
		public $char_values_list;
						*/
						...
						//print_r($list_caracteristic_values);
						 ...
						 $this->query.= " (SELECT products.id, products.product_article, products.category_belong, products.product_price, products.nds_out, products.product_visible, products.product_name , products.product_parent_id, products.product_vitrina, products.product_html_description FROM products_temp WHERE products.category_belong = ".$this->show_group." UNION SELECT products.id, products.product_article, categories_products.group AS category_belong, products.product_price, products.nds_out, products.product_visible, products.product_name , products.product_parent_id, products.product_vitrina, products.product_html_description FROM products_temp WHERE categories_products.group = ".$this->show_group." JOIN categories_products ON categories_products.product = products.id) ";
