...
<div style="background-color:#F0F0F0; float:left;width:295px; padding:1px"><strong>Товары в группе</strong>
  ...
  <table   width="300" border="0" cellspacing="2" cellpadding="2" background="/images/2x2.png" style="max-width:300px; overflow:auto"><?
for ($i=0; $i<count($products_in_gr); $i++ ) {
  ...
  <?
	...
	echo " style=\"max-width: 200px;overflow: hidden\">".CHtml::link( $products_in_gr[$i]->product_name,array('adminproducts/product', 'id'=>$products_in_gr[$i]->id, 'group'=>$group, 'char_filter'=>Yii::app()->getRequest()->getParam('char_filter')), $htmlOptions=array ('encode'=>false, 'style'=>'font-family: Arial Narrow; text-decoration:none') )."</td>
  </tr>";
...
$tab->tabs=array(
	...
	'tab11'=>array(
			'title'=>'Видео с YouTube/vimeo',
			//'view'=>'product-img-add',
			'view'=>'video',
			'data'=>array('product'=>$product, 'group'=>$group, 'videos'=>null ),
	),
	...
	'tab6'=>array(
          'title'=>'Рекомендуемые товары/Группы',
		  'view'=>'product-recomend',
          'data'=>array('product'=>$product, 'group'=>$group, 'compabile'=>$compabile, 'compabile_categories'=>$compabile_categories),
    ),
