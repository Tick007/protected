...
<table width="100%" border="0"  cellpadding="0" cellspacing="0">
  ...
  <tr bgcolor="#CFC7AD">
    <td>Клиент</td>
    <td colspan="4" bgcolor="#CFC7AD"><strong><?=$order->client->second_name.' '.$order->client->first_name.' '.$order->client->last_name?></strong>, <?=$order->client->client_email?>, <?= CHtml::link('правка', array('/roles/details','uid'=>$order->client->id) , array('onclick'=>"return hs.htmlExpand(this, { objectType: 'iframe' } )"));
  ...
  <tr bgcolor="#E9E5D9">
    ...
    <td colspan="4"><?
		...
		?><ul>
        <?php
        for($i=0; $i<count($orde-r>payments); $i++){
			echo '<li>';
