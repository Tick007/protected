...
<thead>
        <tr bgcolor="#EAE5D8"  class="fixed">
          <th colspan="4" align="left">Пользователь:&nbsp;<?php
          if(isset($user) && $user!=null) echo $user->login?></th> 
          </tr>
        </thead>
        <tr bgcolor="#CFC7AD">
    <td>Логин</td>
 ...
 <tr bgcolor="#CFC7AD">
   ...
   <tr bgcolor="#E9E5D9">
    ...
    <tr bgcolor="#E9E5D9">
      ...
      <td colspan="3" bgcolor="#CFC7AD"><?php
    echo CHtml::textField('main_user_params[client_city]',@$user->client_city, $htmlOptions=array('encode'=>false, 'size'=>30) );
    if(is_numeric(@$user->client_city)) {
    	echo '&nbsp;';
    	$CITY = World_adres_cities::model()->findByPk(@$user->client_city);
    	if($CITY != null) echo $CITY->name.', '.$CITY->region->name;
    }
	?></td>
  </tr>
    <tr bgcolor="#E9E5D9">
      <td bgcolor="#CFC7AD">Адрес</td>
      <td colspan="3" bgcolor="#CFC7AD"><?php
    echo CHtml::textField('main_user_params[client_street]',@$user->client_street, $htmlOptions=array('encode'=>false, 'size'=>30) );
	?></td>
    </tr>
    <tr bgcolor="#E9E5D9">
