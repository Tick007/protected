    ...
    public function registerClientScripts()
    {
 	  ...
 	  public function registerCssFile($url=null)
	  {
        ...
        if ($this->baseUrl === '')
	  	  ...
	  	  $cs=Yii::app()->getClientScript();
	  	  if($url===null) { 
		  	//echo Yii::app()->theme->name;
	  		 // $url=$this->baseUrl.'/themes/'.Yii::app()->theme->name.'/css/mbmenu.css';
			  $url='/themes/'.Yii::app()->theme->name.'/css/mbmenu.css?v='.rand();
          		//echo $url;
		  $cs->registerCssFile($url,'screen');
          $browser = Browser::detect();
          if ($browser['name'] == 'msie' && $browser['version'] < 8)
            $cs->registerCssFile('/themes/'.Yii::app()->theme->name.'/css/mbmenu_iestyles.css','screen');
        } else {
	  	    $cs->registerCssFile($url,'screen');
        }
	  }
	  ...
	  protected function normalizeItems($items,$route,&$active, $ischild=0)
	  {
	  	foreach($items as $i=>$item)
	  	{
	  		...
	  		if($this->encodeLabel)
	  			if (isset($item['label']))$items[$i]['label']=CHtml::encode($item['label']);
				else {
			//print_r($item);
			//exit();
			$items[$i]['label'] =$item[0];
	  		}
