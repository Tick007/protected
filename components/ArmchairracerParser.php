...
class ArmchairracerParser extends SParserController
{
	...
	if($ItemB = $parser->parseInnerTag('div', array('id' => 'navBreadCrumb')))////////див бредкрамбса
		...
		$tmp_3 = $parser->parseTag('a', array(), $ItemB,  4, $pattern);
			if(isset($tmp_3) AND is_array($tmp_3)==true) $tmp3 = $tmp_3[1];
		
		
		if(!$result['parentCategoryName'] = $this->clean($tmp3[1]))
			return $this->createParseError('Невозможно извлечь родительскую категорию.', $url);
			
		if($result['parentCategoryName']==='CARS' AND $result['categoryName']=='Машинки Ninco' ) $result['parentCategoryName']='Ninco';
		if($result['parentCategoryName']==='CARS' AND $result['categoryName']=='Машинки SCX' ) $result['parentCategoryName']='SCX';
		if($result['parentCategoryName']==='CARS' AND $result['categoryName']=='Машинки Scalextric' ) $result['parentCategoryName']='Scalextric';
		...
		if(!empty($tmp8) && isset($tmp8[0])) {
				//print_r($tmp8[0]);
				$tmp8[0] = str_replace('style="mso-special-character:line-break"', '', $tmp8[0]);///////////бяка
				$tmp8[0] = str_replace("<br />", '<br>', $tmp8[0]);///////////бяка
				$tmp8[0] = str_replace('<br  />', '<br>', $tmp8[0]);///////////бяка
				$tmp8[0] = str_replace('<br/>', '<br>', $tmp8[0]);///////////бяка
				$tmp8[0] = str_replace('1:32', '1/32', $tmp8[0]);///////////бяка
				//print_r($tmp8[0]);
				
				$tmp4 = $parser->parse('%<p.*?>(.*?)</p>%is', $tmp8[0]);
				
				//print_r($tmp4[1]);
				
				if(isset($tmp4[1])){
						$chars =  preg_split ("[<br>]", $tmp4[1]);
						
						if(isset($chars) ) for($i=0; $i<count($chars); $i++) {
							$qqq  = explode(':', $this->hardClean($chars[$i]) );
							//echo $chars[$i].':  ';
							//print_r($qqq);
							//echo '<br>';
							if(trim(  $this->hardClean(str_replace('/>', '', strtolower($qqq[0]))))  AND trim($this->translate_chars($this->hardClean(str_replace('/>', '', strtolower($qqq[0]))))) AND trim($this->translate_values(trim(str_replace('-', '',$this->hardClean($qqq[1])))))  )$result['characteristics'][$this->translate_chars($this->hardClean(str_replace('/>', '', strtolower($qqq[0]))))]=$this->translate_values(trim(str_replace('-', '',$this->hardClean($qqq[1]))));
						}
				}
		}
		...
		// Цена
		$result['price'] = null;
		if($result['price'] = $parser->parseInnerTag('h2', array('id' => 'productPrices')))
			$result['price'] = trim(str_replace('$', '', $this->clean($result['price']))*32);
		
		// Артикул
		$result['articul'] = null;
		$pattern= '%<li>(.*?)<\/li>%is';
		$productDetailsList = $parser->parseInnerTag('ul', array('id' => 'productDetailsList'));
		$qqq = $parser->parse($pattern, $productDetailsList, 4);
		if(isset($qqq[1])) {  
					$chars = $qqq[1];
					for($i=0; $i<count($chars); $i++) {
							$qqq  = explode(':', $this->hardClean($chars[$i]) );
							if($this->hardClean(str_replace('/>', '', strtolower($qqq[0]))) != 'model') $result['characteristics'][$this->translate_chars($this->hardClean(str_replace('/>', '', strtolower($qqq[0]))))]=$this->translate_values(trim(str_replace('-', '',$this->hardClean($qqq[1]))));
							else $result['articul'] = trim(str_replace('-', '',$this->hardClean($qqq[1])));
						}
		}
		...
		$tmp9 = $parser->parseTagAttribute('a', 'href', array('rel' => 'lightbox'));
