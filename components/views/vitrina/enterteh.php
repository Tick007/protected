...
<h3><?php echo $title?></h3>
	...
	for ($i=0; $i<count($products); $i++) {
		...
		<tr>
    <td valign="top" height="125" align="center">
    	<?php
							$iconname = Yii::app()->request->baseUrl."/pictures/add/icons/".$products[$i]->icon.'.png';
									//echo $_SERVER['DOCUMENT_ROOT'].$iconname;
									if (file_exists($_SERVER['DOCUMENT_ROOT'].$iconname)==1) echo CHtml::link("<img src=\"$iconname\" class=\"content_img\" />", array('product/details', 'alias'=>$products[$i]->belong_category->alias, 'pd'=>$products[$i]->id));
