...
$fp = fopen($_SERVER['DOCUMENT_ROOT'].'/yandex_market.xml', 'w');
...
<categories>
	...
	for ($k=0; $k<count($categories); $k++) {
		...
		if($categories[$k]->parent==Yii::app()->params['main_tree_root']) $buf= '<category id="'.$categories[$k]->category_id.'">'.htmlspecialchars($categories[$k]->category_name).'</category>';
		elseif(trim($categories[$k]->category_name)!='') $buf= '<category id="'.$categories[$k]->category_id.'" parentId="'.$categories[$k]->parent.'">'.htmlspecialchars($categories[$k]->category_name).'</category>';
		fwrite($fp,  $buf);
	}
	ob_start();
	?>
